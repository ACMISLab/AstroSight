#!/usr/bin/env python3
"""
Galaxy Morphology Classification using Qwen-VL-Max
星系形态分类实验 - 使用通义千问视觉模型
"""
...
class GalaxyClassifier:
    ...
    def run_evaluation(self, test_file: str, output_file: str = None, sample_size: int = None):
        ...
        for i, data_item in enumerate(tqdm(test_data, desc="Classifying galaxies")):
            try:
                ...
                # 注释掉中间保存，所有结果最后一起保存
                # if (i + 1) % 50 == 0:
                #     self.save_intermediate_results(detailed_results, f"intermediate_results_{i+1}.json")
    ...
    def print_results_summary(self, results: Dict):
        """打印结果摘要"""
        print("\n" + "="*60)
        print("GALAXY CLASSIFICATION RESULTS - QWEN-VL-MAX")
        print("="*60)
...
def main():
    """主函数"""
    # 配置参数
    API_KEY = "sk-6ec32429c5fa4c68a87a57c8c687709d"
    TEST_FILE = "/remote-home/cs_acmis_hby/code/FT-LLM/galaxy_classification/dataset_07/test.jsonl"
    OUTPUT_FILE = "qwen_vl_galaxy_classification_results.json"
    
    # 设置样本数量（用于测试，设为None使用全部数据）
    SAMPLE_SIZE = None  # 先用100个样本测试，后续可以改为None使用全部数据
    
    print("Starting Galaxy Classification Experiment with Qwen-VL-Max")
    print(f"API Key: {API_KEY[:10]}...")
    print(f"Test file: {TEST_FILE}")
    print(f"Sample size: {SAMPLE_SIZE}")
    
    # 创建分类器
    classifier = GalaxyClassifier(API_KEY)

#!/usr/bin/env python3
"""
Galaxy Morphology Classification using GLM-4.5V (智谱AI)
星系形态分类实验 - 使用智谱AI GLM-4.5V视觉模型（新品旗舰版）
"""
...
class GLM45VGalaxyClassifier:
    ...
    def classify_galaxy(self, image_path: str, max_retries: int = 1) -> Tuple[str, str]:
        """
        分类单张星系图片
        ...
        for attempt in range(max_retries):
            try:
                ...
                data = {
                    "model": "glm-4.5v",  # 回退到稳定的GLM-4V视觉模型
                    "messages": [
                        {
                            "role": "user",
                            "content": [
                    ...
                    "max_tokens": 1500,  # 大幅增加token限制确保完整响应
                    "temperature": 0.6,
                    "thinking": {
                        "type": "disabled"  # 关闭thinking模式，获得简洁响应
                    }
                }
                ...
                if response.status_code == 200:
                    ...
                    if 'choices' in result and len(result['choices']) > 0:
                        ...
                        # GLM-4.5V支持thinking模式，内容可能在reasoning_content中
                        response_text = message.get('content', '')
                        reasoning_text = message.get('reasoning_content', '')
                        
                        # 优先使用content，如果为空则使用reasoning_content
                        final_response = response_text if response_text.strip() else reasoning_text
                        
                        logger.info(f"Raw response: {final_response}")
                        
                        # 提取预测标签
                        predicted_label = self.extract_label(final_response)
                        
                        if predicted_label:
                            return predicted_label, final_response
                        else:
                            logger.warning(f"Could not extract valid label from response: {final_response}")
                            # 不再重试，直接失败
                            break
                    else:
    ...
    def extract_label(self, response: str) -> str:
        ...
        # 处理GLM特殊格式 <|observation|> 等
        if "<|OBSERVATION|>" in response:
            logger.warning("Received observation token, skipping...")
            return None
        ...
        # 查找 "X:" 格式 (如 "A:round elliptical")
        import re
        pattern = r'\b([ABCDEFGH]):'
        match = re.search(pattern, response)
        if match:
            return match.group(1)
        
        # GLM-4.5V的thinking模式需要特殊处理
        # 查找类似 "答案是 X" 或 "选择 X" 或 "answer is X" 的模式
        thinking_patterns = [
            r'答案是\s*([ABCDEFGH])',
            r'选择\s*([ABCDEFGH])',
            r'answer\s+is\s+([ABCDEFGH])',
            r'choice\s+is\s+([ABCDEFGH])',
            r'选项\s*([ABCDEFGH])',
            r'option\s+([ABCDEFGH])',
            r'我选择\s*([ABCDEFGH])',
            r'I\s+choose\s+([ABCDEFGH])',
            r'结果是\s*([ABCDEFGH])',
            r'result\s+is\s+([ABCDEFGH])'
        ]
        ...
        # 如果thinking模式匹配失败，查找独立的字母（但要避免常见单词）
        # 排除常见单词中的字母，如 "Got", "Figure", "Galaxy", "First" 等
        words_to_exclude = ['GOT', 'FIGURE', 'GALAXY', 'FIRST', 'BASED', 'EDGE', 'BARRED', 'HELP']
        words = response.split()
        for word in reversed(words):  # 从后往前找，更可能是答案
            clean_word = re.sub(r'[^A-H]', '', word)
            if len(clean_word) == 1 and clean_word in 'ABCDEFGH':
        ...
        logger.warning(f"Could not extract label from: {response}")
        return None
    ...
    def print_results_summary(self, results: Dict):
        """打印结果摘要"""
        print("\n" + "="*60)
        print("GALAXY CLASSIFICATION RESULTS - GLM-4.5V (智谱AI)")
        print("="*60)
...
def main():
    """主函数"""
    # 配置参数  
    API_KEY = "631b0cfe16944a52a252d4c05182f949.VP0BVKJ2bvQ9cTjF"
    TEST_FILE = "/remote-home/cs_acmis_hby/code/FT-LLM/galaxy_classification/dataset_07/test.jsonl"
    OUTPUT_FILE = "glm45v_galaxy_classification_results.json"  # GLM-4.5V全量实验结果
    
    # 设置样本数量（用于测试，设为None使用全部数据）
    SAMPLE_SIZE = None  # 全量实验 - 11,088个样本
    
    print("Starting Galaxy Classification Experiment with GLM-4.5V (智谱AI)")
    print(f"API Key: {API_KEY[:10]}...")
    print(f"Test file: {TEST_FILE}")
    print(f"Sample size: {SAMPLE_SIZE}")
    
    # 创建分类器
    classifier = GLM45VGalaxyClassifier(API_KEY)

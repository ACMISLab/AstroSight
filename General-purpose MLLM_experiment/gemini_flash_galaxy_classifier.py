#!/usr/bin/env python3
"""
Galaxy Morphology Classification using Gemini-2.5-flash (Google AI)
星系形态分类实验 - 使用Google AI Gemini-2.5-flash视觉模型
"""
...
class GeminiFlashGalaxyClassifier:
    ...
    def print_results_summary(self, results: Dict):
        """打印结果摘要"""
        print("\n" + "="*60)
        print("GALAXY CLASSIFICATION RESULTS - Gemini-2.5-flash (Google AI)")
        print("="*60)
...
def main():
    ...
    OUTPUT_FILE = "gemini_flash_galaxy_classification_results.json"
    
    # 设置样本数量（用于测试，设为None使用全部数据）
    SAMPLE_SIZE = None  # 运行完整测试集 - 11,088个样本
    
    print("Starting Galaxy Classification Experiment with Gemini-2.5-flash (Google AI)")
    print(f"API Key: {API_KEY[:10]}...")
    print(f"Test file: {TEST_FILE}")
    print(f"Sample size: {SAMPLE_SIZE}")
    
    # 创建分类器
    classifier = GeminiFlashGalaxyClassifier(API_KEY)
